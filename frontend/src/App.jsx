...
function App() {
   ...
   * IMPORTANT:
   *
   * This state belongs to App instead of Home.
   *
   * App stays mounted while you navigate between pages,
   * so the Home intro will not restart when Home is mounted again.
   *
   * A full browser refresh recreates App from scratch,
   * so this becomes false again and the book plays again.
   */
  ...
  const renderPage = () => {
     ...
     * HOME
     *
     * The intro is shown only while homeIntroShown === false.
     *
     * Once Home calls onIntroComplete(), App changes
     * homeIntroShown to true.
     *
     * Going to another page and returning to Home does NOT
     * reset homeIntroShown.
     *
     * Refreshing the browser DOES reset it because React
     * state starts over after a real page reload.
     */
    return (
      <Home
        onNavigate={handleNavigate}
        showIntro={!homeIntroShown}
